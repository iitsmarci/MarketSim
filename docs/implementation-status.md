...
| 7-20                                    | Not started | Persistence, PWA, native platforms, advanced models, and release implementation remain future work                                 |
...
Milestone 7 may begin only after explicit approval. The deferred M6 browser
matrix should be run when an environment with local-server access is available;
it does not change the implemented mathematical or runtime contract.
